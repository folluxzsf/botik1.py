...
# Словарь для хранения ожидаемых ответов от бота Akemi
    akemi_sync_waiters: dict[int, asyncio.Event] = {}
    akemi_sync_data: dict[int, dict] = {}


def parse_akemi_leaderboard(embed: discord.Embed) -> dict | None:
    """Парсит данные лидерборда из embed бота Akemi"""
    try:
        data = {}
...
# Конвертируем голосовое время в XP (5 XP за минуту)
        ...
        print(f"[Akemi Sync] Ошибка парсинга embed: {e}")
        return None


def parse_akemi_leaderboard_text(text: str) -> dict | None:
    """Парсит данные лидерборда из текста сообщения бота Akemi"""
    try:
        data = {}
        import re
...
# Конвертируем голосовое время в XP
        ...
        print(f"[Akemi Sync] Ошибка парсинга текста: {e}")
        return None
...
async def setvoice_command(ctx: commands.Context, user_input: str, duration: str):
    if not is_super_admin(ctx.author):
        await ctx.send(
        embed=make_embed(
        "Нет доступа",
...
# Парсим пользователя или ID
        user_id = None
        member = None
        user_mention = None

# Пытаемся получить пользователя по упоминанию или ID
    try:
# Пробуем получить как Member (если на сервере)
...
# Пытаемся получить пользователя для отображения
    ...
    if user_id is None:
        ...
        embed=make_embed(
        "Ошибка",
        "Не удалось определить пользователя. Используй упоминание или ID пользователя.",
        color=0xED4245,
        )
        )
        return
    ...
    if seconds is None:
        ...
        "Используй формат `ЧЧ.ММ.СС` (например, `12.30.15`).",
        color=0xFEE75C,
        )
        )
        return
        ...
        f"✅ {user_mention} теперь имеет {format_voice_duration_from_seconds(seconds)} в голосе "
...
@bot.command(name="asynx")
async def asynx_command(ctx: commands.Context):
    """Синхронизирует данные из лидерборда бота Akemi в levels.json

    Команда отправляет /leaderboard боту Akemi и парсит ответ для обновления данных.
    """
    if not ctx.guild:
        await ctx.send(embed=make_embed("Команда только для сервера", "Используйте команду на сервере.", color=0xED4245))
        return
...
# Проверка прав (только супер-админ)
    if not is_super_admin(ctx.author):
        await ctx.send(embed=make_embed(
        "Нет доступа",
        "🚫 Только супер-администратор может использовать эту команду.",
        color=0xED4245
        ))
        return
        ...
        await ctx.send(embed=make_embed(
        "⏳ Синхронизация",
        "Отправляю команду `/leaderboard` боту Akemi...\n"
        "Пожалуйста, убедитесь, что бот Akemi находится на сервере и может отвечать в этом канале.",
        color=0xFEE75C
        ))
...
# Создаем событие для ожидания ответа
        ...
        event = asyncio.Event()
        akemi_sync_waiters[channel_id] = event
